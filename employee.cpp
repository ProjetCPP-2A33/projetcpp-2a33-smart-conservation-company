...
bool Employee::exporterPDF(const QString &nomFichier, QAbstractItemModel *model) {
    ...
    // Configuration du texte et des styles
    painter.setPen(Qt::black);
    painter.setFont(QFont("Arial", 20));
    painter.drawText(2000, 1000, "Liste des Employés");

    // Position et taille des cellules
    int startX = 200;
    int startY = 1500;
    int cellWidth = 1200;
    int cellHeight = 600;
    ...
    QStringList headers = {"ID", "Nom", "Prenom", "Login", "Mot de Passe", "Téléphone", "Poste", "Disponibilité", "Heure de Travail"};
    painter.setFont(QFont("Arial", 12, QFont::Bold));
    for (int col = 0; col < headers.size(); ++col) {
        painter.drawText(startX + col * cellWidth, startY, cellWidth, cellHeight, Qt::AlignCenter, headers[col]);
    }

    // Dessiner les lignes et les bordures de la table
    painter.drawRect(startX, startY, headers.size() * cellWidth, cellHeight);
    ...
    for (int row = 0; row < rowCount; ++row) {
        ...
        int currentRowY = startY + (row + 1) * cellHeight;

        for (int col = 0; col < headers.size(); ++col) {
            QString data = model->data(model->index(row, col)).toString();
            QRect cellRect(startX + col * cellWidth, currentRowY, cellWidth, cellHeight);
